    ...
    async def async_set_value(self, device_id: int, thing_id: int, r_code: str, value: int) -> bool:
        ...
        for attempt in range(retries):
            try:
                ...
                _LOGGER.debug("Invio scrittura (tentativo %d/%d) a URL: %s con payload: %s", attempt + 1, retries, url, payload)
                ...
                async with self._session.post(url, headers=headers, json=payload, timeout=20) as response:
                    ...
                    if response.status == 400:
                        ...
                        if "Bad Response from device" in response_text:
                            _LOGGER.warning(
                                "Ricevuto 'Bad Response from device' per %s. Riprovo tra 2 secondi... (Tentativo %d/%d)",
                                r_code,
                                attempt + 1,
                                retries,
                            )
                            last_exception = EmmetiApiClientError(f"Bad Response from device dopo {retries} tentativi")
                ...
                last_exception = e
                _LOGGER.error("Errore durante il tentativo %d di scrittura del valore per %s: %s", attempt + 1, r_code, e)
                if attempt < retries - 1:
                    await asyncio.sleep(2)
                else:
                    _LOGGER.error("Falliti tutti i tentativi di scrittura per %s.", r_code)
                    break
        
        _LOGGER.error("Errore finale durante la scrittura del valore per %s: %s", r_code, last_exception)
        return False